...
re_test_gt = pd.read_csv('data/SOTAB/raw/RE/RE_Test/CPA_test_gt.csv')
another_csv = pd.read_csv('data/SOTAB/raw/RE/RE_Test/CPA_test_random_gt.csv')
...
for file in os.listdir('data/SOTAB/raw/RE/RE_Test/Test/'):
    df, ct = read_table(f'data/SOTAB/raw/RE/RE_Test/Test/{file}')
    if len(df) < _min:
        _min = len(df)
    if len(df) > _max:
        _max = len(df)
    if df is not None and file in gt['test']:
        ...
        tokens[file] = ct
label_set = list(re_test_gt['label'].unique())
