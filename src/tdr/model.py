...
class BertCLSAvgPooler(nn.Module):
    ...
    def forward(self, hidden_states, input_ids):
        ...
        expanded_cls_mask = cls_mask.unsqueeze(-1).expand(-1, -1, hidden_states.size(-1))
        cls_token_states = hidden_states * expanded_cls_mask
        cls_sum = cls_token_states.sum(dim=1)  # Sum over the sequence length dimension
        cls_count = cls_mask.sum(dim=1, keepdim=True)  # Count [CLS] tokens per sequence
        cls_count = cls_count + (cls_count == 0).float()
        cls_avg = cls_sum / cls_count
        pooled_outputs = self.dense(cls_avg)
        pooled_outputs = self.activation(pooled_outputs)

        return pooled_outputs

class BertSingPooler(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.dense = nn.Linear(config.hidden_size, config.hidden_size)
        self.activation = nn.Tanh()
    ...
    def forward(self, hidden_states, input_ids):
        ...
        seq_length = non_padding_mask.sum(dim=1) - 1
        seq_length = seq_length.long()
        batch_indices = torch.arange(hidden_states.size(0), dtype=torch.long, device=hidden_states.device)
        last_token_tensor = hidden_states[batch_indices, seq_length]

        pooled_outputs = self.dense(last_token_tensor)
        pooled_outputs = self.activation(pooled_outputs)
...
class BertModelSingleOutput(BertPreTrainedModel):
    """
    ...
    def __init__(self, config):
        ...
        # Freeze the parameters in the embeddings and encoder
        ...
        # TODO: FOR SA
        # self.pooler = BertCLSAvgPooler(config)
        self.pooler = BertSingPooler(config)
    ...
    Examples::
        ...
        #TODO
        orig_mask = extended_attention_mask
        ...
        pooled_output = self.pooler(sequence_output, input_ids).squeeze(
            0)  # (1, N, 768) => (1, 768): single token representation
        # =====================================================
